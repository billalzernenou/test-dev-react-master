...
class RegistrationForm extends React.Component {
  ...
  render() {
    const isEmailError =
      undefined !== this.state.errors.email && this.state.touched.email;
    const isPasswordError =
      undefined !== this.state.errors.password && this.state.touched.password;
    const isConfirmError =
      undefined !== this.state.errors.confirm && this.state.touched.confirm;
      ...
      <div className="RegisterForm">
        ...
        <form onSubmit={this.handleSubmit.bind(this)}>
          <div className={`form-item${isEmailError ? " has-error" : ""}`}>
            ...
            {isEmailError && (
              <span className="error-message">
                {this.state.errors.email.map((err, k) => (
          ...
          <div className={`form-item${isPasswordError ? " has-error" : ""}`}>
            ...
            {isPasswordError && (
              <span className="error-message">
                {this.state.errors.password.map((err, k) => (
                  <span key={k} className="error-item">
                    {err}
                  </span>
                ))}
          ...
          <div className={`form-item${isConfirmError ? " has-error" : ""}`}>
            ...
            <input
              ...
              onChange={(e) => this.handleInputChange(e, "confirm")}
            />
            {isConfirmError && (
              <span className="error-message">
                {this.state.errors.confirm.map((err, k) => (
                  <span key={k} className="error-item">
                    {err}
                  </span>
                ))}
